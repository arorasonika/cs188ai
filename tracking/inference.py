...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        picked = random.uniform(0, self.total())
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost_pos in self.allPositions:
            obs_prob = self.getObservationProb(observation, pacman_pos, ghost_pos, jail_pos)
            self.beliefs[ghost_pos] = self.beliefs[ghost_pos] * obs_prob
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            temp_beliefs[particle] += self.getObservationProb(observation,
                                                              gameState.getPacmanPosition(),
                                                              particle,
                                                              self.getJailPosition())

        if temp_beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            distr = self.getPositionDistribution(gameState, p)
            new_p = distr.sample()
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for g in range(self.numGhosts):
                ...
                value *= self.getObservationProb(observation[g],
                                                              gameState.getPacmanPosition(),
                                                              particle[g],
                                                              self.getJailPosition(g))
            temp_beliefs[particle] += value
