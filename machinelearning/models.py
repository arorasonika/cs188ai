...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        dot_product_scalar = nn.as_scalar(nn.DotProduct(self.w, x))
    ...
    def train(self, dataset):
        ...
        while True:
            ...
            for x, y in dataset.iterate_once(1):
                ...
                pred_y = self.get_prediction(x)
                predicted_ys += [pred_y]
                real_ys += [scalar_y]
                if pred_y != scalar_y:
                    self.w.update(x, scalar_y)
            if predicted_ys == real_ys:
                return
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        step1 = nn.Linear(x, self.m1)
        step2 = nn.AddBias(step1, self.b1)
        step3 = nn.ReLU(step2)
        step4 = nn.Linear(step3, self.m2)
        final_step = nn.AddBias(step4, self.b2)
        return final_step
    ...
    def train(self, dataset):
        ...
        """
        "*** YOUR CODE HERE ***"
        batch_size = int(dataset.x.shape[0]/2)
        while True:
            scalar_loss = float("Inf")
            for x, y in dataset.iterate_once(batch_size):
                gradients = nn.gradients(self.get_loss(x, y), [self.m1, self.m2, self.b1, self.b2])
                ...
                scalar_loss = nn.as_scalar(self.get_loss(x, y))
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        step3 = nn.ReLU(step2)
        step4 = nn.Linear(step3, self.m2)
        step5 = nn.AddBias(step4, self.b2)
        step6 = nn.ReLU(step5)
        step7 = nn.Linear(step6, self.m3)
        final_step = nn.AddBias(step7, self.b3)
        return final_step
